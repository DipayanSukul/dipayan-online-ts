...
import { GiphyAuthor } from '../../../hooks/giphy/types';
...
export const GifAuthorCredits: React.FC<Props> = ({ classes: receivedClasses = {}, user }) => {
	...
	return (
		<a className={classes.container} target="_blank" rel="noreferrer" onClick={clickTrap} href={user.profileUrl}>
			{user.profileAvatarUrl && <img className={classes.img} src={user.profileAvatarUrl} alt={user.name} />}
