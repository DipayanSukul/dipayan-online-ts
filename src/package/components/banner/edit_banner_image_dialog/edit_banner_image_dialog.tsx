...
interface unsplashButtonProps {
	setSearchUnsplashDialogOpened: () => void;
	hasUnsplashEndpoint: boolean;
}

interface editBannerImageDialogComponentProps {
	open: boolean;
	onClose: () => void;
...
const UnsplashButton: React.FC<unsplashButtonProps> = ({ setSearchUnsplashDialogOpened, hasUnsplashEndpoint }) => {
...
const EditBannerImageDialogComponent: React.FC<editBannerImageDialogComponentProps> = ({ open, onClose, onChange }) => {
	...
	const needsFailoverField = useMemo(
	...
	const onImageSelected = useCallback(
		...
		[onChange, onClose, setSearchUnsplashDialogClosed]
	);
	...
	const onDrop = useCallback(
		(files) =>
			onFilesUpload(files).then((url: string) => {
				onImageSelected({ url });
				return url;
			}),
		[onImageSelected, onFilesUpload]
	);
